...
def safety_verdict(rc):
    ...
    # 5. LP locked — varning men inte critical på Pump.fun
    if rc["lp_pct"] >= 80:
        passes.append(f"✓ LP Locked {rc['lp_pct']}%")
    elif rc["lp_pct"] >= 50:
        passes.append(f"✓ LP delvis låst {rc['lp_pct']}%")
    elif rc["lp_pct"] >= 20:
        fails.append(f"⚠️ LP låst {rc['lp_pct']}% — lågt")
    else:
        fails.append(f"⚠️ LP ej låst ({rc['lp_pct']}%) — kolla manuellt!")
